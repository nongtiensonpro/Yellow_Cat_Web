...
import {Users, Package, BarChart2, Moon, Sun, UserCheck, UserX, Shield, User, ActivityIcon, Percent, Box, Tag, Layers, Shirt} from "lucide-react"; // Import new icons
import Link from "next/link";
import { useState, useEffect } from "react";
import { useSession } from 'next-auth/react';
import LoadingSpinner from '@/components/LoadingSpinner';
...
// New interfaces for detailed product categories stats
interface GenericTotalStats {
    total: number;
}
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        const fetchProductStats = async () => {
            try {
                ...
                console.error("Lỗi khi lấy thống kê sản phẩm:", e);
                setProductStats({
                    total: 0,
                    active: 0,
                    inactive: 0,
    ...
    useEffect(() => {
        const fetchPromotionStats = async () => {
            try {
                const accessToken = session?.accessToken;
                const response = await fetch('http://localhost:8080/api/promotions', { // Adjust as needed
                    headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : {},
                });
                if (!response.ok) throw new Error("Không thể lấy dữ liệu khuyến mãi");
                const data = await response.json();
    ...
    // --- NEW: Stats for detailed product categories ---
    const [shoeSoleStats, setShoeSoleStats] = useState<GenericTotalStats>({ total: 0 });
    const [shoeTypeStats, setShoeTypeStats] = useState<GenericTotalStats>({ total: 0 });
    const [materialStats, setMaterialStats] = useState<GenericTotalStats>({ total: 0 });
    const [brandStats, setBrandStats] = useState<GenericTotalStats>({ total: 0 });

    const fetchDataForCategory = async (url: string, setState: React.Dispatch<React.SetStateAction<GenericTotalStats>>, categoryName: string) => {
        try {
            const accessToken = session?.accessToken;
            const response = await fetch(url, {
                headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : {},
            });
            if (!response.ok) throw new Error(`Không thể lấy dữ liệu ${categoryName}`);
            const data = await response.json();
            // Assuming the API returns a 'total' or 'count' field, or is an array whose length is the total
            const totalCount = Array.isArray(data) ? data.length : data.total || data.count || 0;
            setState({ total: totalCount });
        } catch (e) {
            console.error(`Lỗi khi lấy thống kê ${categoryName}:`, e);
            setState({ total: 0 });
        }
    };
    ...
    useEffect(() => {
        if (status === "authenticated") {
            ...
            // fetchDataForCategory('http://localhost:8080/api/shoe-soles/total', setShoeSoleStats, 'đế giày'); // REPLACE WITH YOUR ACTUAL API
            // fetchDataForCategory('http://localhost:8080/api/shoe-types/total', setShoeTypeStats, 'loại giày'); // REPLACE WITH YOUR ACTUAL API
            fetchDataForCategory('http://localhost:8080/api/materials/total', setMaterialStats, 'chất liệu');
            fetchDataForCategory('http://localhost:8080/api/brands/total', setBrandStats, 'thương hiệu');
        }
    }, [session, status]);
    // --- END NEW STATS ---
        ...
        <div className={`min-h-screen py-8 px-4 md:px-36 transition-colors ${darkMode ? "dark bg-gray-900" : "bg-gray-50"}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
                ...
                <Card className="shadow-lg">
                    ...
                    <CardFooter>
                        <Button as={Link} href="/admin/product_management" color="success" variant="flat" size="sm" className="w-full">
                            Quản lý sản phẩm
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex items-center gap-3">
                        <Avatar icon={<Layers size={24} />} color="secondary" /> {/* Using Layers icon for shoe soles */}
                        <div className="flex-1">
                            <p className="text-lg font-semibold text-gray-700 dark:text-white">Đế giày</p>
                            <p className="text-2xl font-bold text-purple-600 dark:text-purple-300">{shoeSoleStats.total}</p>
                        </div>
                    </CardHeader>
                    <CardFooter>
                        <Button as={Link} href="/admin/product_management/shoe-soles" color="secondary" variant="flat" size="sm" className="w-full">
                            Quản lý đế giày
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex items-center gap-3">
                        <Avatar icon={<Shirt size={24} />}  /> {/* Using Shirt icon for shoe types */}
                        <div className="flex-1">
                            <p className="text-lg font-semibold text-gray-700 dark:text-white">Loại giày</p>
                            <p className="text-2xl font-bold text-cyan-600 dark:text-cyan-300">{shoeTypeStats.total}</p>
                        </div>
                    </CardHeader>
                    <CardFooter>
                        <Button as={Link} href="/admin/product_management/shoe-types"  variant="flat" size="sm" className="w-full">
                            Quản lý loại giày
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex items-center gap-3">
                        <Avatar icon={<Box size={24} />}  /> {/* Using Box icon for materials */}
                        <div className="flex-1">
                            <p className="text-lg font-semibold text-gray-700 dark:text-white">Chất liệu</p>
                            <p className="text-2xl font-bold text-pink-600 dark:text-pink-300">{materialStats.total}</p>
                        </div>
                    </CardHeader>
                    <CardFooter>
                        <Button as={Link} href="/admin/product_management/materials" variant="flat" size="sm" className="w-full">
                            Quản lý chất liệu
                        </Button>
                    </CardFooter>
                </Card>

                {/* NEW CARD: Thương hiệu */}
                <Card className="shadow-lg">
                    <CardHeader className="flex items-center gap-3">
                        <Avatar icon={<Tag size={24} />}  /> {/* Using Tag icon for brands */}
                        <div className="flex-1">
                            <p className="text-lg font-semibold text-gray-700 dark:text-white">Thương hiệu</p>
                            <p className="text-2xl font-bold text-orange-600 dark:text-orange-300">{brandStats.total}</p>
                        </div>
                    </CardHeader>
                    <CardFooter>
                        <Button as={Link} href="/admin/product_management/brands" variant="flat" size="sm" className="w-full">
                            Quản lý thương hiệu
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-lg font-semibold text-gray-700 dark:text-white">Đơn hàng</p>
                            <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-300">{stats.orders.toLocaleString("vi-VN")}</p>
                        </div>
                    </CardHeader>
                    <CardFooter>
                        <Button as={Link} href="/admin/order_management" color="warning" variant="flat" size="sm" className="w-full">
                            Quản lý đơn hàng
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-lg font-semibold text-gray-700 dark:text-white">Bán hàng</p>
                            <p className="text-2xl font-bold text-gray-600 dark:text-gray-300">N/A</p> {/* No specific total for 'Sales' usually */}
                        </div>
                    </CardHeader>
                    <CardBody className="pt-0">
                        <p className="text-sm text-gray-600 dark:text-gray-300">Chọn loại hình bán hàng:</p>
                    </CardBody>
                    <CardFooter className="flex flex-col gap-2">
                        <Button as={Link} href="/staff/officesales" color="warning" variant="flat" size="sm" className="w-full">
                            Bán hàng tại quầy
                        </Button>
                        <Button as={Link} href="/staff/page.tsx" color="warning" variant="flat" size="sm" className="w-full">
                            Bán hàng online
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    ...
                    <CardFooter>
                        <Button as={Link} href="/admin/revenue" color="danger" variant="flat" size="sm" className="w-full">
                            Xem chi tiết
                        </Button>
                    </CardFooter>
                </Card>
                ...
                <Card className="shadow-lg">
                    ...
                    <CardFooter>
                        <Button as={Link} href="/admin/promotion_management" variant="flat" size="sm" className="w-full">
                            Quản lý khuyến mãi
                        </Button>
                    </CardFooter>
                </Card>
