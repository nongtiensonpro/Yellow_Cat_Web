...
//         }
//         try {
//             const res = await fetch(`${API_URL}/api/promotion-products/${id}`, {
//                 method: 'DELETE',
//                 headers: { Authorization: `Bearer ${session.accessToken}` },
//             });
//             if (res.ok) {
...
//                 alert(`Không thể xoá: ${errorData.message || 'Lỗi không xác định.'}`);
//             }
//         } catch (err) {
//             console.error(err);
...
export default function PromotionManagementPage() {
    ...
    const [filters, setFilters] = useState({
        keyword: '',
        status: '',
        startDateFilter: '', // Thêm state cho ngày bắt đầu lọc
        endDateFilter: '',   // Thêm state cho ngày kết thúc lọc
    });
    const { data: session, status: sessionStatus } = useSession() as { data: CustomSession | null; status: string };
    const [loading, setLoading] = useState(true);
    const [currentPage, setCurrentPage] = useState(1);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/promotion-products/summaries?${queryParams}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const data: APIResponse = await res.json();
            let arr: RawPromotion[] = [];
    ...
    }, [session, sessionStatus, API_URL, filters]);
    ...
    const filteredPromotions = useMemo(() => {
        return promotions.filter(promo => {
            ...
            const startMatch = filters.startDateFilter ? new Date(promo.startDate.split('T')[0]) >= new Date(filters.startDateFilter) : true;
            const endMatch = filters.endDateFilter ? new Date(promo.endDate.split('T')[0]) <= new Date(filters.endDateFilter) : true;
    ...
    const handleResetFilters = () => {
        setFilters({ keyword: '', status: '', startDateFilter: '', endDateFilter: '' });
        setCurrentPage(1);
    };
        ...
        <div className="p-4 md:p-8 space-y-8 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {/* Thêm input lọc theo ngày bắt đầu */}
                    <div className="flex items-center gap-2">
                        <label className="text-gray-500 text-sm whitespace-nowrap">Từ ngày:</label>
                        <input
                            type="date"
                            className="w-full border-gray-300 rounded-lg bg-gray-50 px-4 py-2.5 focus:bg-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition"
                            value={filters.startDateFilter}
                            onChange={e => setFilters({ ...filters, startDateFilter: e.target.value })}
                        />
                    </div>
                    {/* Thêm input lọc theo ngày kết thúc */}
                    <div className="flex items-center gap-2">
                        <label className="text-gray-500 text-sm whitespace-nowrap">Đến ngày:</label>
                        <input
                            type="date"
                            className="w-full border-gray-300 rounded-lg bg-gray-50 px-4 py-2.5 focus:bg-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition"
                            value={filters.endDateFilter}
                            onChange={e => setFilters({ ...filters, endDateFilter: e.target.value })}
                        />
                    </div>
                </div>
