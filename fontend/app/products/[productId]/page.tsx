...
interface ProductVariant {
    ...
    salePrice?: number | null; // Giá khuyến mãi (có thể null)
    stockLevel: number;
    imageUrl: string;
    weight: number;
}
            ...
            if (typeof window !== 'undefined') {
                ...
                if (existing) {
                    ...
                    localCart.push({
                        ...
                        productId: product.productId,
                        name: product.productName,
                        price: selectedVariant.salePrice || selectedVariant.price, // Dùng giá sale nếu có
                        quantity: 1,
                        imageUrl: selectedVariant.imageUrl,
                        sku: selectedVariant.sku,
                        stockLevel: selectedVariant.stockLevel,
                        ...
                        <div className="space-y-6">
                            <div>
                                {selectedVariant && selectedVariant.salePrice && selectedVariant.salePrice < selectedVariant.price ? (
                                    ...
                                    // Trường hợp KHÔNG CÓ GIÁ SALE (hoặc chưa chọn biến thể)
                                    <h2 className="text-3xl font-bold text-red-600">
                                        {selectedVariant ? formatPrice(selectedVariant.price) : (product.variants.length > 0 && product.variants[0].price ? formatPrice(product.variants[0].price) : 'Liên hệ')}
                                    </h2>
                                )}
                                <p className="text-sm text-gray-500 mt-1">
                                    Còn
                                ...
                                <div className="p-4">
                                    {selectedVariant ? (
                                        <table className="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tr>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Giá</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatPrice(selectedVariant.salePrice || selectedVariant.price)}</td>
                                            </tr>
                                            <tr>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Cân nặng</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{selectedVariant.weight} kg</td>
