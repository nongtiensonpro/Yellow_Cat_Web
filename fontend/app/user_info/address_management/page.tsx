...
import {
    ...
    Edit,
    Trash2,
    // ArrowLeft, // Removed ArrowLeft import
} from "lucide-react";
import LoadingSpinner from '@/components/LoadingSpinner';
import AddressForm from './components/AddressForm';
import {
...
export default function AddressManagement() {
        ...
        try {
            ...
            if (apiResponse.data && Array.isArray(apiResponse.data.content)) {
                ...
                const formattedData = apiResponse.data.content.map(address => {
                    ...
                    const decodeVietnamese = (str: string) => {
                        try {
                            // Only decode if it looks like an encoded string (e.g., contains %)
                            if (str.includes('%')) {
                                return decodeURIComponent(escape(str));
                            }
                            return str;
    ...
    const handleDeleteSingle = async (addressId: number) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/addresses/user/delete`, {
                ...
                body: JSON.stringify([addressId]) // Gửi mảng chứa một ID
            });
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                {/* Removed the Button for ArrowLeft and its surrounding div */}
                <h1 className="text-2xl font-bold">Quản lý địa chỉ</h1>
            ...
            >
                <TableHeader>
                    <TableColumn>
                        <Checkbox
                            isSelected={selectedAddresses.length > 0 && selectedAddresses.length === currentItems.length}
                            isIndeterminate={selectedAddresses.length > 0 && selectedAddresses.length < currentItems.length}
                            onValueChange={handleSelectAll}
                        />
                    </TableColumn>
                ...
                <TableBody emptyContent="Không có dữ liệu">
                    {currentItems.map((address, index) => {
                        ...
                        console.log('Rendering address row:', address, 'STT:', stt);
                        return (
                            <TableRow key={address.addressId}>
                                <TableCell>
                                    <Checkbox
