...
const ProductList = () => {
    ...
    const [reviewStatsMap, setReviewStatsMap] = useState<Record<number, {
                ...
                <Link href={`/products/${product.productId}`}>
                    <CardBody className="px-4 py-3">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2 min-w-0">
                                {product.logoPublicId && (
                                    <div
                                        className="w-6 h-6 rounded-full overflow-hidden flex-shrink-0 border border-default-200">
                                        <CldImage width={24} height={24} src={product.logoPublicId}
                                                  alt={product.brandName || 'Thương hiệu'}
                                                  className="w-full h-full object-cover"/>
                                    </div>
                                )}
                                <span
                                    className="text-xs font-medium text-default-700 truncate">{product.brandName}</span>
                            </div>
                        ...
                        <div className="flex items-center gap-2 mt-1"
                            ...
                            <div className={`flex items-center ${isStatsLoading ? 'animate-pulse' : ''}`} role="img"
                                 aria-hidden="true">
                            ...
                            <span
                                className="text-xs text-default-500">{isStatsLoading ? 'Đang tải…' : `${averageRating.toFixed(1)} (${totalReviews})`}</span>
                        </div>
                                    ...
                                    <span className="text-base text-gray-400 line-through">
                                        ...
                                        {!!discountPercent && (
                                            <Chip color="danger" variant="flat" size="sm"
                                                  className="ml-1">-{discountPercent}%</Chip>
                                        )}
                                    </>
                                ) : (
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <div className="flex items-center gap-2 text-default-600">
                                ...
                                <Chip size="sm" variant="flat"
                                      color={product.totalStock && product.totalStock > 10 ? 'success' : product.totalStock && product.totalStock > 0 ? 'warning' : 'danger'}>
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            <h2 className="text-3xl font-bold mb-6">Tất cả sản phẩm của chúng tôi</h2>
