...
public class ProductController {
    ...
    @GetMapping("/best-selling-custom") // <-- API endpoint mới
    @Operation(summary = "Get best-selling products by custom date range", description = "Returns a list of products ordered by units sold within a specified date range.")
    @PreAuthorize("hasAnyAuthority('Admin_Web')") // Chỉ admin mới được dùng
    public ResponseEntity<?> getBestSellingProductsCustom(
            @RequestParam String startDate,
            @RequestParam String endDate
    ) {
        try {
            List<ProductBestSellerCustomDTO> products = productService.getBestSellingProductsCustom(startDate, endDate);
            return ResponseEntityBuilder.success(products);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntityBuilder.error(
                    HttpStatus.INTERNAL_SERVER_ERROR,
                    "Error retrieving custom best-selling products",
                    e.getMessage()
            );
        }
    }
