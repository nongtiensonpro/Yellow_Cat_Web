...
public class ProductController {
    ...
    public ResponseEntity<?> getAllProducts(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(required = false) List<String> brandName, // Thêm tham số lọc thương hiệu
            @RequestParam(required = false) String categoryName,   // Thêm tham số lọc danh mục
            @RequestParam(required = false) Double minPrice,       // Thêm tham số lọc giá tối thiểu
            @RequestParam(required = false) Double maxPrice,       // Thêm tham số lọc giá tối đa
            @RequestParam(required = false) List<String> sizes     // Thêm tham số lọc kích thước
    ) {
        try {
            Pageable pageable = PageRequest.of(page, size);
            // Truyền tất cả các tham số lọc xuống ProductService
            Page<ProductListItemDTO> productPage = productService.getProductsPaginated(
                    pageable, brandName, categoryName, minPrice, maxPrice, sizes
            );
            return ResponseEntityBuilder.success(productPage);
        } catch (Exception e) {
            e.printStackTrace();
