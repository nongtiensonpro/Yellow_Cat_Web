...
public class ProductService {
    ...
    // Phương thức này có vẻ không dùng hoặc là một phương thức chung, giữ nguyên
    Page<ProductDto> getAllProducts(Pageable pageable) {
        Page<Product> products = productRepository.findAll(pageable);
        return products.map(productMapper::toDto);
    }
    ...
    // =======================================================================
    // SỬA ĐỔI PHƯƠNG THỨC GETPRODUCTSPAGINATED ĐỂ HỖ TRỢ LỌC
    // =======================================================================
    public Page<ProductListItemDTO> getProductsPaginated(
            Pageable pageable,
        ...
        // Vì bạn đang dùng Pageable và sẽ truyền trực tiếp Pageable vào Repository
        // Spring Data JPA sẽ tự động tính pageSize và offset.
        // Nó cũng sẽ tự động sử dụng countQuery bạn đã định nghĩa trong @Query
        // để đếm tổng số sản phẩm sau khi lọc.

        // Gọi repository với tất cả các tham số lọc và Pageable
        // LƯU Ý: ĐIỀU NÀY YÊU CẦU BẠN SỬA LẠI findAllProductsPaginated TRONG ProductRepository
        // để nó nhận Pageable và trả về Page<ProductListItemDTO> thay vì List<ProductListItemDTO>
        // và pageSize/offset thủ công.
        // Tôi sẽ sửa lại ProductRepository để phù hợp với cách tiếp cận này.
        // Nếu không, bạn phải giữ nguyên cách dùng pageSize/offset và gọi count riêng.

        // Cách tiếp cận KHUYÊN DÙNG: ProductRepository trả về Page
        return productRepository.findAllProductsFiltered(
                brandNames,
                categoryName,
                minPrice,
                maxPrice,
    ...
    // =======================================================================
    // CÁC PHƯƠNG THỨC KHÁC KHÔNG THAY ĐỔI VÌ KHÔNG LIÊN QUAN ĐẾN LỌC
    // =======================================================================
