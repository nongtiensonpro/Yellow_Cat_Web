    ...
    public CartConfirmResponseDTO confirmCartItems(UUID keycloakId, List<ProductConfirmDTO> selectedProducts) {
        ...
        try {
            ...
            for (ProductConfirmDTO selected : selectedProducts) {
                ...
                if (variant.getQuantityInStock() < 10) {
                    System.out.println("Sản phẩm '" + variant.getProduct().getProductName()
                            + "' hiện chỉ còn ở cửa hàng, vui lòng liên hệ hotline để biết thêm chi tiết");
                }
            ...
            for (ProductVariant variant : variantsToUpdate) {
                ...
                System.out.println("Số lượng trong kho"+ variant.getQuantityInStock());
                System.out.println("Số lượng trừ đi"+ quantity);
                System.out.println("Số lượng còn lại"+ variant.getQuantityInStock());
