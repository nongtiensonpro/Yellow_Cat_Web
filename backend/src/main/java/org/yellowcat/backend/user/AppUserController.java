...
public class AppUserController {
    ...
    public ResponseEntity<?> getAllUsers(@RequestParam(defaultValue = "0") int page,
                                         @RequestParam(defaultValue = "10") int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<AppUserDTO> userPage = appUserService.findAll(pageable);
        return ResponseEntityBuilder.success(new PageResponse<>(userPage));
    }

//    @GetMapping("/{id}")
//    public ResponseEntity<?> getUserById(@PathVariable Integer id) {
//        return appUserService.findById(id)
//                .map(user -> ResponseEntityBuilder.success(new AppUserDTO(user)))
//                .orElseGet(() -> ResponseEntityBuilder.error(HttpStatus.NOT_FOUND, "Người dùng không tồn tại", null));
//    }
    ...
    @PostMapping("/create/{key}")
    public ApiResponse createUser(@RequestBody AppUserDTO user, @PathVariable String key) {
        AppUserDTO createdUser = appUserService.createUser(user, key);
        return ApiResponse.success(createdUser);
    }

    @PutMapping("/update/{key}")
    public ApiResponse updateUser(@RequestBody AppUserDTO user, @PathVariable String key) {
        AppUserDTO updatedUser = appUserService.updateUser(user, key);
    ...
    public ResponseEntity<?> deleteUser(@PathVariable Integer id) {
        boolean isDeleted = appUserService.delete(id);
        if (isDeleted) {
            return ResponseEntityBuilder.success("Người dùng đã được xóa thành công");
        } else {
            return ResponseEntityBuilder.error(HttpStatus.BAD_REQUEST, "Xóa thất bại", "Người dùng không tồn tại hoặc đã bị xóa rồi");
        }
    }
    ...
    public ResponseEntity<?> checkEmailExists(@RequestParam String email) {
        ...
        return ResponseEntity.ok("Email hợp lệ, có thể sử dụng");
    }
